const passports = `eyr:2028 iyr:2016 byr:1995 ecl:oth
...
pid:#725759
hcl:#602927 iyr:2013 byr:2003 eyr:2023 cid:100`.split(/\n\n/g);

let validpassport: number = 0;

let newpassportArr: Array<Array<string>> = [];
let passportRecorder: Record<string, boolean | undefined> = {};
let validpassportsArr: Array<Array<string>> = [];
interface Ipassport {
  byr: number;
  iyr: number;
  eyr: number;
...
let validpolicies: number = 0;

passports.forEach((e, i) => {
  newpassportArr.push(passports[i].split(/\s+/));
});

newpassportArr.forEach((e, i) => {
  let founcid = e.find((i) => i.includes('cid'));
  passportRecorder[i] = founcid ? true : false;

  if (passportRecorder[i] === true) {
    if (e.length === 8) {
      ++validpassport;
      validpassportsArr.push(e.sort());
    }
  } else if (passportRecorder[i] === false) {
    if (e.length === 7) {
      ++validpassport;
      validpassportsArr.push(e.sort());
    }
...
// console.log(validpassportsArr)
let passportsObj: Array<Ipassport> = [];
validpassportsArr.forEach((e, i) => {
  let byr: string | undefined = e.find((i) => i.includes('byr'));
  let ecl: string | undefined = e.find((i) => i.includes('ecl'));
  let eyr: string | undefined = e.find((i) => i.includes('eyr'));
  let hcl: string | undefined = e.find((i) => i.includes('hcl'));
  let hgt: string | undefined = e.find((i) => i.includes('hgt'));
  let iyr: string | undefined = e.find((i) => i.includes('iyr'));
  let pid: string | undefined = e.find((i) => i.includes('pid'));

  if (byr && ecl && hcl && hgt && pid && eyr && iyr) {
    byr = byr.slice(4);
    ecl = ecl.slice(4);
    eyr = eyr.slice(4);
    hcl = hcl.slice(4);
    ...
    pid = pid.slice(4);
    iyr = iyr.slice(4);
    let messure = hgt.slice(hgt.length - 2);
    let passport: Ipassport = {
      ...
      ecl,
      hcl,
      hgt: parseInt(hgt.slice(0, hgt.length - 2)),
      pid,
    };

    let hclregex = new RegExp(/^[#][0-9a-f]{6}$/g);
    let pidregex = new RegExp(/^[0-9]{9}$/g);
    let options = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
    function verifyoptions(word: string): boolean{
    ...
    if (messure === 'cm') {
      if (
        ...
        passport.hgt >=150 &&
        passport.hgt <= 193 &&
        passport.hcl.match(hclregex) &&
        verifyoptions(passport.ecl) &&
        passport.pid.match(pidregex)
      ) {
        ++validpolicies;
      }
    } else if (messure === 'in') {
