const passwords: string = `3-4 j: tjjj
...
const passwordsArr: string[] = passwords.split('\n');
// console.log(passwordsArr);

let passed: number = 0;
let newpassed: number = 0;

for (let pass of passwordsArr) {
  let password: string[] = pass.split(': ');
  let letter = password[0].slice(password[0].length - 1).trim();
  let condition = password[0].slice(0, password[0].length - 2).trim().split('-');
  let characters = password[1].split(letter).length - 1;

  let passwordsArr = password[1].split('');
  if(passwordsArr[parseInt(condition[0]) - 1] === letter && passwordsArr[parseInt(condition[1]) - 1] === letter){
    
  }else if(passwordsArr[parseInt(condition[0]) - 1] === letter || passwordsArr[parseInt(condition[1]) - 1] === letter){
      ++newpassed;
  }
  if (
    characters >= parseInt(condition[0]) &&
    characters <= parseInt(condition[1])
  ) {
    ++passed;
  }
}
